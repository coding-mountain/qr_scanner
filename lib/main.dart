...
import 'package:qr_scan_test/scanner.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<Database> openCustomDatabase() async {
    ...
    final String path = join(databasesPath, 'doggie_database.db');
    return await openDatabase(path);
  }
  Future logDatabase() async{
    final Database db = await openCustomDatabase();
    final List<Map<String, dynamic>> rows = await db.query('employees');
    debugPrint("Database length  : ${rows.length}");
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
